...
void meta_print(struct packet_meta meta, __u64 timestamp)
{
...
int print_event(struct perf_event_sample *sample)
{
    ...
    // if (sample->metadata.cookie != 0xdead) {
	// 	printf("BUG cookie %x\n", sample->metadata.cookie);
	// 	return LIBBPF_PERF_EVENT_ERROR;
	// }

	/* print payload hex */
	meta_print(sample->metadata, sample->timestamp);
    printf("\t");
...
int main(int argc, char **argv)
{
	...
	struct bpf_prog_load_attr prog_load_attr = {
		...
		.file = "xdppacket_kern.o",
	};
	struct bpf_map *perf_map;
	struct bpf_object *obj;
	int sys_fds[MAX_CPUS];
	int perf_map_fd;
	int prog_fd;
	int n_cpus;
	int opt;
